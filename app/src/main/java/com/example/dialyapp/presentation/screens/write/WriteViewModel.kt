...
import com.example.dialyapp.util.toRealmInstant
...
class WriteViewModel @Inject constructor(
    ...
    private val imageToUploadDao: ImagesToUploadDao,
    ...
    private fun getDiaryIdArgument() {
        uiState = uiState.copy(
            selectedDiaryId = savedStateHandle.get<String>(WRITE_SCREEN_ARGUMENT_KEY)
        )
    }
    ...
    fun deleteDiary(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (uiState.selectedDiaryId != null) {
                val result = MongoDB.deleteDiary(ObjectId.from(uiState.selectedDiaryId!!))
                if (result is RequestState.Success) {
                    withContext(Dispatchers.Main) {
